...
echo ""
    ...
    echo ""
    echo "    TOOL-CALL PROXY (recommended for tool-calling clients like opencode):"
    echo "    The MLX server emits tool calls as <tools> XML text rather than"
    echo "    structured tool_calls. A proxy at scripts/mlx-proxy.ts converts it:"
    echo "      npm run mlx:proxy"
    echo "    Then point clients at http://localhost:8084/v1 (default proxy port)."
    echo "    Set MLX_HOST=http://localhost:8084 to route the framework through the proxy."
