...
description: >-
  ...
  reads story from the planning board, plans tasks, analyzes codebase, generates code,
...
3. **`branchPattern`**:
   ...
   - **YourProject**: `{teamPrefix}{env}/{storyNumber}_{slug}` — underscore before slug; **`teamPrefix`** from `teamPrefixes[<planning board team OID>]` (e.g. `Team:2002` → `ninjas/`); **`env`** from the dev-site environment picked in the assign flow (lowercased, e.g. `donatello`). Example: `ninjas/donatello/b-17010_front_end_step`.
...
3. Create only **frontend tasks** in the planning board (UI components, state management, API integration)
...
6. Before creating anything, check `.frontend-status.json` for existing tasks for this story. Reuse existing task numbers when the name/category already matches; do not recreate duplicate tasks on a resumed Phase 1 run.
